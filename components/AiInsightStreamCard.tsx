...
import { Sparkles, Lightbulb, ArrowRight, Activity } from "lucide-react";
import { useInsightStream } from "@/features/insight/hooks/useInsightStream";

interface AiInsightStreamCardProps {
  storeId: string;
  storeName: string;
  periodLabel?: string;
  insightUrl?: string;
...
export function AiInsightStreamCard({
  storeId,
  storeName,
  periodLabel = "4–31 Agu 2026",
  insightUrl = "#",
}: AiInsightStreamCardProps) {
  const { text, status, source, errorMsg } = useInsightStream(storeId, storeName);

  const isStreaming = status === "streaming";
  const defaultHeadline = "Performa toko Anda menunjukkan tren positif.";
  const defaultNarrative =
    "Dalam 28 hari terakhir (4–31 Agu 2026), toko mencatat 11 transaksi dengan total omzet Rp2.823.000. Produk Gift Box Anniversary Deluxe menjadi kontributor pendapatan terbesar.";
  ...
  // If text is streamed, we display the streamed text nicely
  const displayNarrative = text ? text : defaultNarrative;
        ...
        <div className="flex-1 space-y-3.5 max-w-3xl">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-1.5 text-xs font-semibold text-[#6355D9]">
              ...
              {isStreaming && (
                <span className="inline-flex items-center gap-1 text-[10px] text-violet-600 bg-violet-100 px-2 py-0.5 rounded-full font-normal">
                  <Activity className="size-3 animate-pulse text-violet-600" />
                  Menganalisis...
                </span>
              )}
            </div>
          ...
          <div className="space-y-1">
            ...
            <div className="text-xs text-[#57534E] leading-relaxed whitespace-pre-line">
              {displayNarrative}
            </div>
          </div>
          ...
          {/* Footnote metadata */}
          <div className="text-[10px] text-[#A8A29E] flex items-center gap-2 pt-1">
            <span>Sumber: {source || "data transaksi"}</span>
            {errorMsg && <span className="text-amber-600">({errorMsg})</span>}
            <span>• Semua angka dihitung dari data transaksi</span>
          </div>
        </div>
